...
def call_claude_session_log(
    *,
    ...
    system_content = (
        f"You are a campaign keeper for an ongoing D&D 5e campaign. The campaign's "
        f"setting, party, and recurring cast are described in the campaign context "
        f"below — use it for names, places, and tone. Your task is to write a "
        f"detailed, accurate session log for Session {session_label} based on the "
        f"transcript provided by the user.\n\n"
        f"## Required output format\n\n"
        f"Produce a single Markdown document with this exact structure. Do not emit "
        f"YAML frontmatter — start directly with the H1 heading:\n\n"
        f"```\n"
        f"# Session {session_label} — <invented title based on main events>\n"
        f"**Date played:**\n"
