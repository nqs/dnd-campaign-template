...
- [World](World) — setting overview, cosmology, timeline, tone
- [Geography](Geography) — regions, cities, travel distances, climate
- [Factions](Factions) — organizations, their goals, their conflicts
- [Roster](Roster) — NPCs and relationships
- [Party](Party) — current PCs, levels, classes, backstories, goals
- [Session Log](Session-Log) — what's happened so far, loose ends, foreshadowing

## Sessions

- *(add session links here as you run them)*
- Example: `[Session 001 — The Opening Hook](the-opening-hook-1-adventure)`
...
- [Campaign Keeper instructions](Agents) — source hierarchy, canon-first rules, generator handoff
- [Adventure Generator workflow](DnD-Adventure-Generator) — scope → outline → images → markdown → bible → PDF
...
Obsidian wikilinks like `[[roster]]` resolve regardless of folder, so bullets here work whether the target is at root or under `campaign/`. Path-prefixed wikilinks (e.g. `[[sessions/session 1/...]]`) point at the root-level `sessions/` tree.
